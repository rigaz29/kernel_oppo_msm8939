...
/* strscpy() is 4.3+; every call here ignores the return value */
#define strscpy(dst, src, size) strlcpy(dst, src, size)
#endif
#if LINUX_VERSION_CODE < KERNEL_VERSION(4, 3, 0)
/* The backported static_key_{true,false} wrap a struct static_key; 3.10's
 * static_key_enabled() only takes the bare key. */
#define susfs_static_key_enabled(x) static_key_enabled(&(x)->key)
#else
#define susfs_static_key_enabled(x) static_key_enabled(x)
#endif
...
#define SUSFS_LOGI(fmt, ...) if (static_branch_likely(&susfs_is_log_enabled)) pr_info("susfs:[%u][%d][%s] " fmt, __kuid_val(current_cred()->uid), current->pid, __func__, ##__VA_ARGS__)
#define SUSFS_LOGE(fmt, ...) if (static_branch_likely(&susfs_is_log_enabled)) pr_err("susfs:[%u][%d][%s]" fmt, __kuid_val(current_cred()->uid), current->pid, __func__, ##__VA_ARGS__)
#else
...
static inline bool is_i_uid_not_allowed(uid_t i_uid) {
	return likely(__kuid_val(current_cred()->uid) != i_uid);
}
...
#else
		if (unlikely(test_bit(AS_FLAGS_SUS_PATH, &fi->inode.i_state) &&
			is_i_uid_not_allowed(__kuid_val(fi->inode.i_uid))))
#endif
		{
...
#else
	if (unlikely(test_bit(AS_FLAGS_SUS_PATH, &inode->i_state) &&
		is_i_uid_not_allowed(__kuid_val(inode->i_uid))))
#endif
	{
		SUSFS_LOGI("hiding path with ino '%lu'\n", inode->i_ino);
		return true;
...
void susfs_set_hide_sus_mnts_for_non_su_procs(void __user **user_info) {
	...
	SUSFS_LOGI("susfs_is_hide_sus_mnts_for_non_su_procs_enabled: %d\n", susfs_static_key_enabled(&susfs_is_hide_sus_mnts_for_non_su_procs_enabled));
	info.err = 0;
out_copy_to_user:
...
void susfs_set_uname(void __user **user_info) {
	...
	if (!susfs_static_key_enabled(&susfs_is_uname_spoof_buffer_set))
...
void susfs_set_cmdline_or_bootconfig(void __user **user_info) {
	...
	if (!susfs_static_key_enabled(&susfs_is_fake_cmdline_or_bootconfig_buffer_set))
			...
			entry->target_dev == inode->i_sb->s_dev)
		{
			switch(entry->info.uid_scheme) {
				case UID_NON_APP_PROC:
					if (__kuid_val(current_cred()->uid) % 100000 < 10000)
						break;
					goto out_srcu_read_unlock;
				case UID_ROOT_PROC_EXCEPT_SU_PROC:
					if (__kuid_val(current_cred()->uid) == 0 && !susfs_is_current_ksu_domain())
						break;
					goto out_srcu_read_unlock;
...
static void susfs_sdcard_cleanup_fn(struct work_struct *work)
{
	...
	if (susfs_static_key_enabled(&susfs_is_sdcard_android_data_not_decrypted))
