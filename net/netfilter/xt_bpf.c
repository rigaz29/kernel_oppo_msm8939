				   ...
				   struct bpf_prog **ret)
{
	...
	/*
	 * A37: dua perubahan sekaligus, mengikuti upstream.
	 *
	 * Pertama, API: sk_unattached_filter_create() menjadi bpf_prog_create().
	 *
	 * Kedua -- dan ini memperbaiki bug: versi lama meng-cast insns menjadi
	 * `struct sock_filter __user *`, padahal array itu ADA DI MEMORI KERNEL
	 * (xtables sudah menyalin seluruh matchinfo dari userspace sebelum
	 * checkentry dipanggil). sk_unattached_filter_create() lalu melakukan
	 * copy_from_user() atas pointer kernel. bpf_prog_create() menerima
	 * sock_fprog_kern dan memperlakukannya sebagai pointer kernel -- yang
	 * memang benar.
	 */
	program.len = len;
