...
/*
 * Dinamai qdss_lock, BUKAN d_lock. Konversi dcache ke lockref memperkenalkan
 * makro di include/linux/dcache.h:104 -- "#define d_lock d_lockref.lock" --
 * sehingga identifier bernama d_lock di berkas mana pun yang menyertakan
 * dcache.h akan tergantikan teksnya dan gagal kompilasi. Berkas ini di-#include
 * langsung ke android.c (android.c:49), jadi ikut terkena.
 * acroreiser memakai nama yang sama untuk alasan yang sama.
 */
